...
$strQuestion = "<p class='name'><a href='{$this->url->create('users/view/'.$data['q']['acronym'])}' class='userlink'>".
    "<img src='http://www.gravatar.com/avatar/{$data['q']['hash']}?d=identicon&s=36' class='gravatar' alt='gravatar'> {$data['q']['name']}</a></p>".
    "<div class='content'>{$this->textFilter->doFilter($data['q']['content'], 'shortcode, markdown')}</div>".
...
foreach ($data['a'] as $a) {
    ...
    $strAnswers .= "<div class='answer'>".
        "<p class='name'><a href='{$this->url->create('users/view/'.$a['acronym'])}' class='userlink'><img src='http://www.gravatar.com/avatar/{$a['hash']}?d=identicon&s=36' class='gravatar' alt='gravatar'> {$a['name']}</a></p>".
        "<div class='content'>{$this->textFilter->doFilter($a['content'], 'shortcode, markdown')}</div>".
    ...
    if (isset($comments[$id])) {
        foreach ($comments[$id] as $c) {
            $html .= "<div class='comment'>".
                "<p><a href='{$di->url->create('users/view/'.$c['acronym'])}' class='userlink'><img src='http://www.gravatar.com/avatar/{$c['hash']}?d=identicon&s=18' class='gravatar' alt='gravatar'> ".
                "<span class='name'>{$c['name']}</span></a> ".
                "<span class='info'>{$c['created']}</span></p> ".
                "<div class='content'>{$di->textFilter->doFilter($c['content'], 'shortcode, markdown')}</div> ".
                "</div>";
        }
    }
