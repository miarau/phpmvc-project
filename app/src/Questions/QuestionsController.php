 ...
 *
 */
class QuestionsController implements \Miax\DI\IInjectionAware
{
    ...
    {
        $this->questions = new \Miax\Questions\Questions();
    ...
    public function addAction()
    {
        ...
        if ($status === true) {
            ...
            if (empty($form->value('title'))) {
                $msg .= "Du måste ange en rubrik<br/><br/>";
                $fieldsOk = false;
            }
            if (empty($form->value('content'))) {
                $msg .= "Du måste ange en fråga<br/><br/>";
                $fieldsOk = false;
            }
            if (empty($_POST['tags'])) {
                $msg .= "Du måste ange minst en tagg<br/><br/>";
                $fieldsOk = false;
            }
    ...
    public function writeAction($type, $ref)
    {
        ...
        if ($status === true) {
            ...
            // Check if all fields are valid ($fieldsOk)
            if (empty($form->value('content'))) {
                $msg .= "Fältet får inte vara tomt<br/>";
                $fieldsOk = false;
            }
            $msg .= "<a href='javascript:history.back();'>Gå tillbaka till formuläret.</a>";
            ...
            if ($type === "A") {
                ...
                $content = "<h1>Svar till: {$this->questions->getTitle($ref)}</h1>".
                    "<div class='qtext'>{$this->textFilter->doFilter($this->questions->getContent($ref), 'shortcode, markdown')}</div>";
            }
            else {
    ...
    private function questionForm($param = 'Q') {
        ...
        $form = new \Mos\HTMLForm\CForm(['legend' => $legend], [
                ...
                'content' => [
                    'type'       => 'textarea',
                    'label'      => 'Text',
                    'description'=> 'Du kan formatera din text med hjälp av <a href="http://daringfireball.net/projects/markdown/syntax" target="_blank">Markdown</a>.'
                ],
                'tags' => [
                    'type'       => 'checkbox-multiple',
                    'values'     => $tags,
                    'description'=> 'Välj rätt taggar för din fråga.'
                ],
                'submit' => [
                    'type'       => 'submit',
                    'value'      => 'Skicka',
